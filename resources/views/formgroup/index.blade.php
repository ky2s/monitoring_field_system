...
@section('header')
<div class="col-md-5">
	<h1>Question List <small>-- {{$form->label}} </small></h1>
</div>
<div class="col-md-7">
	<ol class="breadcrumbs">
		<li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> Home</a></li>
		<li><a href="{{route('projectlist')}}"><i class="fa fa-dashboard"></i> Project</a></li>
		<li><a href="{{route('formlist', $form->form->project->id)}}"><i class="fa fa-dashboard"></i> Form List</a></li>
        <li><a href="{{route('formdetaillist', $form->form->id)}}"><i class="fa fa-dashboard"></i> {{$form->form->name}}</a></li>
		<li class="active"><i class="fa fa-pencil-square-o"></i> {{$form->label}}</li>
	</ol>
</div>
@endsection
...
<div class="col-md-12 col-xs-12">
	<!--<form id="rule2-form" action="{{ route('formdetailrule2', $form->form->id) }}" method="POST">
		{{ csrf_field() }}
	        ...
	        <div class="panel panel-default">
	            <div class="panel-heading flat">
	                <h4 class="panel-title">
						<a data-toggle="collapse" data-parent="#accordion" href="#collapseOne"><b>{{$form->form->name}}</b> <span class="fa fa-right fa-caret-square-o-down pull-right"></span></a>
	                </h4>
	            </div>
		                ...
		                <div class="panel-body flat">
							...
							<div class="body-side-header">
								<form>
									...
									<div>	
										<input type="text" value="{{$form->form->name}}" class="form-control flat" id="basic-url" aria-describedby="basic-addon3" disabled="true">
									</div>

									<label><small>Description :</small></label>
									<div>	
										<textarea disabled="true" name="" id="" cols="30" rows="5" class="form-control flat">{{$form->form->keterangan}}</textarea>
									</div>
									<br>
									<div>
    ...
    <div class="panel panel-{{$pnl}}" id="panel1">
        <div class="panel-heading" style="padding:0px !important;">
            ...
            @endif
              ...
              @if(!$form->form->elock)
              <li><a href="#" onclick="hapus({{$f->id}});">Delete</a></li>
              @endif
            </ul>
          </div>

          <div class="clearfix"></div>
          	<form id="hapus-form{{$f->id}}" action="{{ route('formgroupdestroy', [$f->id, $f->group_id]) }}" method="POST" style="display: none;">
                {{ csrf_field() }}
            </form>
            <form id="rule-form{{$f->id}}" action="{{ route('formgrouprule', [$f->id, $f->group_id]) }}" method="POST" style="display: none;">
                {{ csrf_field() }}
            </form>
	        <h4 class="panel-title">
        ...
        <div id="input-detail-{{$f->id}}" class="panel-collapse collapse">
            <div class="panel-body" style="padding-left:10px; padding-right:10px; padding-top:10px;">
            	<form action="{{URL::route('formgroupupdate', [$f->id, $f->group_id])}}" class="form-horizontal" id="fdetail{{$f->id}}" method="post">
			        {{ csrf_field() }}
	...
	<div class="panel panel-default" id="panel1">
        ...
        <div id="input-detail-new" class="panel-collapse collapse">
            <div class="panel-body" style="padding-left:10px; padding-right:10px">
            	<form action="{{URL::route('formgroupstore', $form->id)}}" class="form-horizontal" id="fdetailnew" method="post">
			        {{ csrf_field() }}
