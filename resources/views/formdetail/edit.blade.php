...
@section('header')
 <div class="col-md-7">
    <h1>Edit Question <small>-- {{$form->name}} <small>-- {{$form->project->name}}</small></small></h1>
</div>
<div class="col-md-5">
    <ol class="breadcrumbs">
        ...
        <li class="active"><i class="fa fa-pencil-square-o"></i> Edit Detail</li>
    </ol>
</div>
@endsection
...
<div class="white-wrapper">
    <form action="{{URL::route('formdetailupdate', [$detailid, $formid])}}" class="form-horizontal" id="fcp" method="post">
        {{ csrf_field() }}
        ...
        <div>
            <input id="mandatory"name="mandatory" class="ace ace-switch ace-switch-3" type="checkbox" {{$detail->mandatory=='Y'?'checked':''}} />
        ...
        <div class="tipechoice">
            ...
            <div>   
                <input type="text" name="option" id="option" class="form-control" 
                        value="{{str_replace('\n', ',', $detail->option)}}" placeholder="Masukkan Pilihan ..." data-role="tagsinput" />
...
@section('jscript')
    ...
    $("#tipetulisan").val("{{$detail->tipetulisan}}");

    $("#ismultiple").val("{{$detail->ismultiple}}");
    $("#maximum").val("{{$detail->maximum}}");
