...
class Formmodel extends Model
{
	...
	public static function getReport($id, $start, $end)
	{
		$table = "inputform".$id;
	...
	public static function getReportExport($id, $start, $end)
	{
		...
		// $query = DB::select($sql);
		$select = array( $table.'.uid', 'users.name as user',
	...
	public static function getReportStat($id)
	{
		$table = "inputform".$id;

		$sql = "SELECT count(id) as jml, to_char(waktuisi,'yyyy-mm-dd') as tgl 
				FROM ".$table."
	...
	public static function getReportStatUser($id)
	{
		$table = "inputform".$id;
	...
	public static function updateData($id, $data)
	{
		...
		// $form->iUpdatedid = Auth::user()->id;
		// $form->tUpdated = date('Y-m-d h:i:s');
	...
	public static function publishData($id)
	{
        $form = Formmodel::find($id);
	...
	public static function lockData($id)
	{
        $form = Formmodel::find($id);
	...
	public static function storeData($data, $id)
	{
		$form = new Formmodel;
	...
	public static function checkData($table, $uid){
		$sql = "SELECT COUNT(id) as jml FROM ".$table." WHERE uid = '".$uid."'";
		$query = DB::select($sql);
		return $query[0]->jml;
	}
