...
class Formmodel extends Model
{
	...
	public static function listMember($id, $page=1)
	{	
		$limit = 300;
		$offset = $limit * ($page - 1);
		$row = Formmodel::select('form.*')
						  ...
						  ->where('team_member.user_id', '=', $id)->orderBy('id')
	...
	public static function getReport($id, $start, $end, $users=null)
	{
		...
		$select = array('users.name', 'users.phone', 'users.email', $table.'.*');
	...
	public static function getReportExport($id, $start, $end)
	{
		...
		// $query = DB::select($sql);
		$select = array( $table.'.uid', $table.'.imei', 'users.name as kode', 'users.phone as phone',
			...
			if(($d->tipe->cfield == 'Y' || $d->tipe->isgroup == 'Y') && $d->group_id ==''){
				if($d->tipe->isgroup == 'Y'){
					foreach($d->group as $f){
						if($f->tipe->cfield == 'Y')
							$select[] = $table.".f".$f->id." as ".$d->label." - ".$f->label;
					}
				}
				else{
	...
	public static function simpanSurvey__($id, $user, $waktu, $latitude, $longitude, $jawaban, $pertanyaanID, $imei, $uid){
		$form = Formmodel::getData($id);
		$fields = "";
		$vals = "";
			...
			if($row->tipe->isgroup =='N'){
				...
				if (in_array($nf, $pertanyaanID)){
					...
					if($row->tipe->id == 2 || $row->tipe->id == 18){
						...
						if(strlen($newJawaban[$nf]) < 255){
							$simp = $id."##".$newJawaban[$nf]."--".$nf."--".$user;
							$sqlsimp = "INSERT INTO aaa (ket) VALUES ('".$simp."');";
							$querysimp = DB::insert($sqlsimp);
						}
		...
		foreach ($form->detail as $row) {
			if($row->tipe->isgroup =='N'){
				...
				if (in_array($nf, $pertanyaanID)){
					...
					$stringQuery[$nf] = $newJawaban[$nf];
				}
			}
		}

		$stringQuery['latitude'] = $latitude;
		$stringQuery['longitude'] = $longitude;
		$stringQuery['waktuisi'] = $waktu;
		$stringQuery['user_id'] = $user;
		$stringQuery['imei'] = $imei;
		$stringQuery['uid'] = $uid;
		// dd($id);
		$lastID = DB::table('inputform'.$id)->insertGetId($stringQuery);
	...
	public static function simpanSurveyImage($userID, $formDetailID, $jawabanBase64, $imei){
		
		if ($userID && $formDetailID && $jawabanBase64){
			
			$formDetail = Formdetailmodel::getData($formDetailID);
			$nf = "f".$formDetailID;
			
			$file = base64_decode($jawabanBase64);
			
			$jpg_url = "survey".$formDetail->form_id."-".time().$imei.$nf.".jpg";

			$path = $formDetail->form_id.'/' . $jpg_url;

			if(strlen($jawabanBase64) < 255){
				$simp = $formDetail->form_id."##".$jawabanBase64."--".$nf."--".$userID;
				$sqlsimp = "INSERT INTO aaa (ket) VALUES ('".$simp."');";
				$querysimp = DB::insert($sqlsimp);
			}
			...
			\Storage::disk('spaces')->put($path, $file, 'public');
			// if($form->id == 25)
				$url_img = \Storage::disk('spaces')->url($path);
				
			// else
				// $vals .= "'".$jpg_url."', ";
				
			$sql = "INSERT INTO public.form_detail_inputform_images (form_detail_id, path, isuploaded, created_at, updated_at) 
					VALUES('".$formDetailID."', '".$url_img."', false, now(), now())";

			$query = DB::insert($sql);

			if($query){				
				$return['url_img'] = $url_img;
				$return['submit_img_id'] = DB::getPdo()->lastInsertId();
			}
			return $return;
		}
	}

	public static function simpanSurveyImageFile($userID, $formDetailID, $jawabanFile, $imei){
		
		if ($userID && $formDetailID && $jawabanFile){
			...
			if(strlen($jawabanFile) < 255){
				$simp = $formDetail->form_id."##".$jawabanFile."--".$nf."--".$userID;
				$sqlsimp = "INSERT INTO aaa (ket) VALUES ('".$simp."');";
				$querysimp = DB::insert($sqlsimp);
			}

			\Storage::disk('spaces')->put($path, file_get_contents($jawabanFile), 'public');
			// if($form->id == 25)
				$url_img = \Storage::disk('spaces')->url($path);
	...
	public static function simpanSurveyImageFileOss($userID, $formDetailID, $jawabanFile, $imei){
		...
		if ($userID && $formDetailID && $jawabanFile){
			...
			\Storage::disk('oss')->put($path, file_get_contents($jawabanFile), 'public');
			// if($form->id == 25)
				$url_img = \Storage::disk('oss')->url($path);
	...
	public static function simpanSurveyImageOssBase64($userID, $formDetailID, $jawabanBase64, $imei){
		...
		if ($userID && $formDetailID && $jawabanBase64){
			...
			$jpg_url = "survey".$formDetail->form_id."-".time().$userID.$imei.$nf.".jpg";
			...
			// Image::make(file_get_contents($file))->save($path);
			// file_put_contents($path, $file);
			\Storage::disk('oss')->put($path, $file, 'public');
			// if($form->id == 25)
				$url_img = \Storage::disk('oss')->url($formDetail->form_id.'/'.$jpg_url);
			...
			$input_form_img_id = DB::table('public.form_detail_inputform_images')->insertGetId($value);

			if(!empty($input_form_img_id)){				
				$return['url_img'] = $url_img;
				$return['submit_img_id'] = $input_form_img_id;
			}
			return $return;
		}
	}

	public static function updateSurveyImage($pertanyaanId, $inputFormImageID, $inputFormID){

		if(!empty($inputFormID)){
			$return = false;
			for($i=0; $i <= count($pertanyaanId); $i++){
				
				if(!empty($inputFormImageID[$i])){
					
					$sql = "UPDATE public.form_detail_inputform_images SET
							isuploaded = true,
							inputform_id = ".$inputFormID.",
							updated_at = now()
							WHERE id =".$inputFormImageID[$i];
					$return = DB::connection('pgsql')->update($sql);
				}
			}
